...
if ( ! function_exists( 'wp_starter_theme_posted_by' ) ) :
	...
	function wp_starter_theme_posted_by() {
		...
		/* translators: 1: posted by label, only visible to screen readers. 2: author link. 3: post author. */
			...
			esc_html__( 'Posted by', 'wp-starter-theme' ),
...
if ( ! function_exists( 'wp_starter_theme_comment_count' ) ) :
	/**
	 * Prints HTML with the comment count for the current post.
	 */
	function wp_starter_theme_comment_count() {
		if ( ! post_password_required() && ( comments_open() || get_comments_number() ) ) {
			/* translators: %s: Name of current post. Only visible to screen readers. */
			comments_popup_link( sprintf( __( 'Leave a comment<span class="sr-only"> on %s</span>', 'wp-starter-theme' ), get_the_title() ) );
		}
	}
endif;

if ( ! function_exists( 'wp_starter_theme_entry_meta' ) ) :
	/**
	 * Prints HTML with meta information for the categories, tags and comments.
	 * This template tag is used in the entry header.
	 */
	function wp_starter_theme_entry_meta() {
		...
		if ( 'post' === get_post_type() ) {
			...
			$categories_list = get_the_category_list( __( ', ', 'wp-starter-theme' ) );
			if ( $categories_list ) {
				printf(
				/* translators: 1: posted in label, only visible to screen readers. 2: list of categories. */
					'<span class="sr-only">%1$s</span>%2$s',
					esc_html__( 'Posted in', 'wp-starter-theme' ),
					$categories_list // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
				);
			}

			/* translators: used between list items, there is a space after the comma. */
			$tags_list = get_the_tag_list( '', __( ', ', 'wp-starter-theme' ) );
			if ( $tags_list ) {
				printf(
				/* translators: 1: tags label, only visible to screen readers. 2: list of tags. */
					'<span class="sr-only">%1$s</span>%2$s',
					esc_html__( 'Tags:', 'wp-starter-theme' ),
		...
		// Comment count.
		if ( ! is_singular() ) {
			wp_starter_theme_comment_count();
		}
		...
		edit_post_link(
			sprintf(
				wp_kses(
				/* translators: %s: Name of current post. Only visible to screen readers. */
					__( 'Edit <span class="sr-only">%s</span>', 'wp-starter-theme' ),
					array(
						'span' => array(
							'class' => array(),
						),
...
if ( ! function_exists( 'wp_starter_theme_post_thumbnail' ) ) :
	...
	function wp_starter_theme_post_thumbnail() {
		if ( ! wp_starter_theme_can_show_post_thumbnail() ) {
			return;
		}
...
if ( ! function_exists( 'wp_starter_theme_comment_avatar' ) ) :
	/**
	 ...
	 */
	function wp_starter_theme_get_user_avatar_markup( $id_or_email = null ) {

		if ( ! isset( $id_or_email ) ) {
			$id_or_email = get_current_user_id();
		}

		return sprintf( '<div class="vcard">%s</div>', get_avatar( $id_or_email, wp_starter_theme_get_avatar_size() ) );
	}
endif;

if ( ! function_exists( 'wp_starter_theme_discussion_avatars_list' ) ) :
	/**
	 * Displays a list of avatars involved in a discussion for a given post.
	 *
	 * @param array $comment_authors Comment authors to list as avatars.
	 */
	function wp_starter_theme_discussion_avatars_list( $comment_authors ) {
		...
		foreach ( $comment_authors as $id_or_email ) {
			printf(
				"<li>%s</li>\n",
				wp_starter_theme_get_user_avatar_markup( $id_or_email ) // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
			);
		}
		echo '</ol>', "\n";
	}
...
if ( ! function_exists( 'wp_starter_theme_the_posts_navigation' ) ) :
	...
	function wp_starter_theme_the_posts_navigation() {
		the_posts_pagination(
			array(
				'mid_size'  => 2,
				'prev_text' => __( 'Newer posts', 'wp-starter-theme' ),
				'next_text' => __( 'Older posts', 'wp-starter-theme' ),
			)
		);
	}
endif;

if ( ! function_exists( 'wp_starter_theme_content_class' ) ) :
	/**
	 ...
	 * WP_STARTER_THEME_TYPOGRAPHY_CLASSES constant.)
	 *
	 ...
	 */
	function wp_starter_theme_content_class( $class = '' ) {
		$all_classes = array( $class, WP_STARTER_THEME_TYPOGRAPHY_CLASSES );
